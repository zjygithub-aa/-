...
#define START_PRIORITY 2  // 启动任务优先级中等，用于创建其他任务
...
#define SENSOR_TASK_PRIORITY 3  // 传感器任务优先级较高，需要定期读取数据

TaskHandle_t SensorTask_handle;

#define COMMAND_TASK_STACK 256  // 命令处理任务栈大小（处理字符串比较等）
#define COMMAND_TASK_PRIORITY 2  // 命令处理优先级中等，响应用户输入
...
void OLED_Task(void *pvParameters)
{
    ...
    OLED_Refresh();
    vTaskDelay(1000 / portTICK_PERIOD_MS);  // 替换systick_ms(1000)
...
void Sensor_Task(void *pvParameters)
{
    for(;;)
    {
        ...
        vTaskDelay(1000 / portTICK_PERIOD_MS);  // 每1秒检查一次，调整延迟以匹配需求
    }
}
...
void Command_Task(void *pvParameters)
{
    for(;;)
    {
        ...
        // key_check();    // 如果需要，取消注释
        vTaskDelay(100 / portTICK_PERIOD_MS);  // 每100ms检查一次flag，调整以避免CPU占用
    }
}
