...
void Init_usart2(void)
{
    ...
    // 启用GPIOG和USART6外设的时钟
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA, ENABLE);  // 启用GPIOG时钟
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART2, ENABLE); // 启用USART6时钟

    // 配置GPIOG的引脚9（TX）和引脚14（RX）为复用功能模式
    ...
    // 配置NVIC以处理USART6的中断
    NVIC_InitStruct.NVIC_IRQChannel = USART2_IRQn;              // 选择USART6中断通道
    ...
    NVIC_Init(&NVIC_InitStruct);                               // 应用NVIC配置

    // 将GPIOG引脚9和引脚14配置为USART6的复用功能
    ...
    // 启用USART6的接收中断并启动
    USART_ITConfig(USART2, USART_IT_RXNE, ENABLE);              // 启用接收非空中断（RXNE）
    USART_Cmd(USART2, ENABLE);                                  // 启用USART2
    USART_ClearFlag(USART2, USART_FLAG_TC);                     // 清除发送完成标志，确保初始状态干净
}

// 函数：重定向标准输出函数fputc，通过USART6发送字符
int fputc(int ch, FILE *f)
{
    USART_SendData(USART2, ch);                                 // 通过USART6发送单个字符
